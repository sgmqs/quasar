...
class NorthstarScraper(Scraper):
    ...
    # Something like if simple get().status = 422, then new_token, else,
    # return existing token.
    def getToken(self, path='/v2/auth/token'):
        """Send OAuth request for new or refresh token."""
        new_token = self.oauth.fetch_token(
            self.url + path, client_id=self.ns_client, client_secret=self.ns_secret, scope='admin')
    ...
    def getUsers(self, users=100, page_number=1):
        ...
        user_response = self.get(
            '/v1/users', {'limit': users, 'page': page_number, 'pagination': 'cursor'})
    ...
    def getUsersSlow(self, users=100, page_number=1):
        ...
        """
        user_response = self.get(
            '/v1/users', {'limit': users, 'page': page_number})
        return(user_response['data'])
