    ...
    for i in profiles:
        for k in profile_data_keys:
            ...
            if profile_data[k][0] == 'child':
                try:
                    profile_data[k][2] = i.find(
                        profile_data[k][1]).find(k).text
                except:
                    profile_data[k][2] = None

        p_data = {'phone': profile_data['phone_number'][0], 'first': profile_data['first_name'][0].replace("'", "").replace('"', ''), 'last': profile_data['last_name'][0].replace("'", "").replace('"', ''), 'email': profile_data['email'][0], 'status': profile_data['status'][0], 'created': profile_data['created_at'][0], 'opted_out_at': profile_data['opted_out_at'][0], 'type': profile_data[
            'type'][2], 'name': profile_data['name'][2], 'street1': profile_data['street1'][2], 'street2': profile_data['street2'][2], 'city': profile_data['city'][2], 'state': profile_data['state'][2], 'postal_code': profile_data['postal_code'][2], 'country': profile_data['country'][2], 'lat': profile_data['latitude'][2], 'long': profile_data['longitude'][2]}
