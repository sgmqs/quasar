...
info = {'status': 'subscribed', 'since_timestamp_opt': datetime.datetime.utcfromtimestamp(origin_time).isoformat(), 'before_timestamp_opt': datetime.datetime.utcfromtimestamp(
    time_now).isoformat(), 'count': 1000, 'fields': 'members.email_address,members.last_changed', 'offset': member_offset}
...
for member in total_members:
    for submember in member:
        ...
        confirm_time = submember['last_changed'].replace(
            'T', ' ').split('+')[0]
        cur.execute(
            "REPLACE INTO users_and_activities.mailchimp_sub_daily (email_address, confirm_time) VALUES(%s,%s)", (email, confirm_time))

# Double Check to Commit DB Changes and Close Connection in case of loop
# failure
db.commit()
cur.close()
db.close()
