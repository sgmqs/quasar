...
def get_demo(obj, db, cur2):
    ...
    # print out
    try:
        q_z_find = "select ` White` as w,  ` Black or African American` as b ,  ` American Indian and Alaska Native` as i,  ` Asian:` as a,`Hispanic Hispanic or Latino (of any race):` as h from  census_tract where st_contains(shape, ST_POINTFROMTEXT('point(%s %s)',2))" % (out[
            0]['long'], out[0]['lat'])
...
def get_gender(obj, db, cur2):
    if obj['first'] is not None:
        ...
        return obj
    try:
        q_g_find = "select gender from gender where first_name = '%s'" % (
...
def get_zip(obj, db, cur2):
    ...
    try:
        ...
        # if obj['postal_code'] is not None:
        ...
        try:
            q_ll_find = "select zcta5ce10 as zip from zips where st_contains(shape, ST_POINTFROMTEXT('point(%s %s)',1))" % (
                llong, lat)
