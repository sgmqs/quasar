...
time_now = time.time()
# Set backfill to seconds and set backfill origin time to time_now -
# backfill_time in seconds
...
info = {'status': 'unsubscribed', 'since_timestamp_opt': datetime.datetime.utcfromtimestamp(origin_time).isoformat(), 'before_timestamp_opt': datetime.datetime.utcfromtimestamp(
    time_now).isoformat(), 'count': 1000, 'fields': 'members.email_address,members.timestamp_opt,members.last_changed', 'offset': member_offset}
...
for member in total_members:
    for submember in member:
        ...
        opt_out_date = submember['timestamp_opt'].replace(
            'T', ' ').split('+')[0]
        confirm_time = submember['last_changed'].replace(
            'T', ' ').split('+')[0]
        cur.execute("REPLACE INTO users_and_activities.mailchimp_unsub_daily (email_address, confirm_time, unsub_time) VALUES(%s,%s,%s)",
                    (email, confirm_time, opt_out_date))
