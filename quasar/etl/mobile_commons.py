...
def get(path, payload):
    ...
    response = requests.get(path, params=payload, auth=(config.mc_user,config.mc_pw))
    return response
...
def _backfill_user_profiles(backfill_hours):
    ...
    while limit_num == 300:
        ...
        payload = {'from' : origin_time_iso, 'to' : now_iso , 'limit': '300', 'page' : page_num}
        ...
        for profile in profiles:
            ...
            if source_type == "Opt-In Path":
                ...
                opt_in_path_id = "NULL"
            insert_profile = "replace into users_and_activities.mobile_user_lookup VALUES ({0}, {1}, {2}, \"{3}\", \"{4}\", \"{5}\", {6}, \"{7}\")".format(phone_number, us_phone_number, uid, created_at, source_type, source_name, opt_in_path_id, status)
