...
class MainSetup:
    ...
    def getCampaigns(self):
        ...
        # dict of manually entered campaigns from mobile_campaign_ids and
        # sms_games
        ...
        for c in campaigns:
            ...
            # add blank opt_in_path. this is needed to catch subscriptions that
            # don't hit an opt in path
            temp_dict = {}
            temp_dict['campaign_status'] = c.attrs.get('active')
            temp_dict['campaign_id'] = campaign_id
            temp_dict['campaign_name'] = c.find('name').string
    ...
    def processClusters(self):
        ...
        for cluster in self.all_clusters:
            for individual_id in cluster['cluster']:
                try:
                    cluster['all_paths'].extend(
                        self.campaigns_dict[individual_id])
                    del self.campaigns_dict[individual_id]
                except Exception as e:
                    print('error', e)
        for remaining in self.campaigns_dict:
            temp_dict = {'cluster': [remaining], 'nid': None,
                         'all_paths': self.campaigns_dict[remaining]}
...
class ClusterSubscribers:
    ...
    def convertTime(self, times_list):
        proc_times_list = [d.strptime(i, '%Y-%m-%d %H:%M:%S %Z')
                           for i in times_list if i is not None and i != '0000-00-00 00:00:00' and i != '']
    ...
    def getSubs(self, page, p, campaign_id, opt_in_id, web_alpha):
        ...
        url_2_payload = {"campaign_id": campaign_id,
                         'opt_in_path_id': opt_in_id, "page": page, "limit": p}
        req = requests.get(url_2, auth=(
            config.mc_user, config.mc_pw), data=url_2_payload)
    ...
    def runPath(self):
        ...
        for path in self.all_paths:
            ...
            if live is True or self.force is True:
                web_alpha = self.isWebAlpha(
                    path['campaign_id'], path['opt_in_id'])
                last_page = self.binarySearch(
                    current_page, path['campaign_id'], path['opt_in_id'])
                page_param = last_page + 1
                p_ret = 100
                last_date = self.getSubs(page_param, p_ret, path[
                                         'campaign_id'], path['opt_in_id'], web_alpha)
                # double check last date by going one page down
                if last_date == d(1970, 1, 1, 0, 0):
                    print('double checking...')
                    dbl_chk_page = page_param - 1
                    last_date = self.getSubs(dbl_chk_page, p_ret, path[
                                             'campaign_id'], path['opt_in_id'], web_alpha)
                    print(last_date)

                if last_date < (d.utcnow() - t(weeks=4)) and self.force is not True:
                    print('not live anymore', path[
           ...
           VALUES ({0},{1},'{2}',{3},'{4}','False') ON DUPLICATE KEY UPDATE last_date='{2}', last_page={3}, active='{4}', live='False'""".format(path['campaign_id'], path['opt_in_id'], d.strftime(last_date, '%Y-%m-%d %H:%M:%S'), last_page, path['campaign_status'])
                    ...
                    while tracker_date > self.stop and int(page_param) > 0:
                        ensure_count -= 1
                        # print path['campaign_id'], path['opt_in_id'],
                        # page_param, tracker_date
                        dt = self.getSubs(page_param, p_ret, path[
                                          'campaign_id'], path['opt_in_id'], web_alpha)
                        tracker_date = dt
